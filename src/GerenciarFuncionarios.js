...
export default function GerenciarFuncionarios() {
  ...
  const abrirEdicao = (func) => {
    ...
    setRg(func.rg || '');
    setCpf(func.cpf || '');
    setNumeroFuncionario(func.numeroFuncionario || '');
    setModalVisible(true);
  };
  ...
  const salvarFuncionario = async () => {
    ...
    const rgFinal = rg.trim();
    const cpfFinal = cpf.trim();
    const numeroFuncionarioFinal = numeroFuncionario.trim();
    ...
    const emailValido = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailFinal);
    if (!emailValido) {
      Alert.alert('Atenção', 'Digite um e-mail válido.');
      return;
    }

    const apenasNumerosCpf = cpfFinal.replace(/\D/g, '');
    if (cpfFinal && apenasNumerosCpf.length !== 11) {
      Alert.alert('Atenção', 'CPF deve ter 11 números.');
      return;
    }

    if (numeroFuncionarioFinal && !/^[A-Za-z0-9_-]+$/.test(numeroFuncionarioFinal)) {
      Alert.alert('Atenção', 'Número do funcionário deve conter apenas letras, números, hífen ou underline.');
      return;
    }

    if (!editandoId && !senhaFinal) {
    ...
    try {
      const payload = {
        ...
        rg: rgFinal,
        cpf: cpfFinal,
        numeroFuncionario: numeroFuncionarioFinal,
      };

      if (senhaFinal) {
        payload.senha = senhaFinal;
      }
 ...
 const excluirFuncionario = async (id) => {
  ...
  try {
    ...
    window.alert(erro.message || 'Não foi possível excluir o funcionário.');
  }
};
  // Filtrar lista
          ...
          listaFiltrada.map((func) => (
            <View key={func.id} style={styles.card}>
              <View style={styles.cardInfo}>
                ...
                <Text style={styles.cardDetalhe}>CPF: {func.cpf || 'Não informado'}</Text>
                <Text style={styles.cardDetalhe}>RG: {func.rg || 'Não informado'}</Text>
                <Text style={styles.cardDetalhe}>Nº funcionário: {func.numeroFuncionario || 'Não informado'}</Text>
              </View>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent={true}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.input} placeholder="Número do Funcionário" value={numeroFuncionario} onChangeText={setNumeroFuncionario} />
...
const styles = StyleSheet.create({
  ...
  cardDetalhe: { fontSize: 11, color: '#777', marginTop: 2 },
