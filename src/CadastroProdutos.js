...
export default function CadastroProdutos() {
  ...
  const handleCadastrarProduto = async () => {
    const precoVendaNumero = parseFloat(precoVenda.replace(',', '.'));
    const precoCustoNumero = precoCusto ? parseFloat(precoCusto.replace(',', '.')) : 0;
    const estoqueNumero = estoque ? parseInt(estoque, 10) : 0;

    if (!nomeProduto.trim() || !categoria || !precoVenda) {
      window.alert('Atenção!\nPreencha os campos obrigatórios (Nome, Categoria e Preço de Venda).');
      return;
    }

    if (Number.isNaN(precoVendaNumero) || precoVendaNumero <= 0) {
      window.alert('Atenção!\nDigite um preço de venda válido e maior que zero.');
      return;
    }
    ...
    if (Number.isNaN(estoqueNumero) || estoqueNumero < 0) {
      window.alert('Atenção!\nDigite uma quantidade de estoque válida.');
      return;
    }
    ...
    try {
      const produto = {
        ...
        categoria: categoria,
        preco_produto: precoVendaNumero,
        preco_custo: precoCustoNumero,
        quantidade_estoque: estoqueNumero,
